...
// Process sets of 50 numbers
function processNumberSets(numbers) {
  const results = [];

  for (let i = 0; i < numbers.length; i += 50) {
    // Get current set of 50 numbers
    const currentSet = numbers.slice(i, i + 50);

    // Skip if we don't have a full set of 50
    if (currentSet.length < 50) break;
    ...
    if (loser) {
      for (let j = i + 50; j < numbers.length; j++) {
    ...
    const countUntilNextLoser =
      nextLoserIndex === -1 ? -1 : nextLoserIndex - (i + 49);
